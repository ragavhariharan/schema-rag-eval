---
name: earthtekniks-vision-calculator
description: >-
  Compute machine-vision engineering values from given parameters, via the
  Calculator Router MCP server. Use this skill whenever the user wants a
  DERIVED / CALCULATED result: field of view, required focal length, working
  distance, depth of field, magnification, exposure time before motion blur,
  line-scan rate, achievable measurement accuracy, sensor geometry, or
  extension-ring length — e.g. "what focal length do I need for a 300mm object
  at 500mm", "can I measure a 0.05mm scratch with my 2448x2048 camera",
  "calculate the FOV", even if the user doesn't say the word "calculate". It
  drives the MCP tools search, get_parent, get_related, list_inputs,
  validate_inputs, and calculate. Use this for COMPUTING values — NOT for
  looking up a product's stored specs (that is the lens catalog skill).
---

> Prerequisite: the **Calculator Router** MCP server must be connected.
